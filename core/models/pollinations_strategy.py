...
이미지를 클릭하면 원본 크기로 볼 수 있습니다."""
    ...
    def _generate_text_response(self, user_input: str, conversation_history: List[Dict] = None) -> str:
        ...
        # 중앙 프롬프트 시스템에서 ASK 모드 프롬프트 가져오기 (언어 지침 포함)
        ask_mode_prompt = prompt_manager.get_system_prompt(ModelType.POLLINATIONS.value, use_tools=False, user_language=user_language)
