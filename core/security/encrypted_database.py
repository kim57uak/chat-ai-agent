...
class EncryptedDatabase:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_encryption_version ON encryption_keys(version)')
    ...
    def _encrypt_data(self, data: str) -> bytes:
        ...
        if not self.auth_manager or not self.auth_manager.is_logged_in():
            security_logger.log_security_violation("encryption_without_auth", "Encryption attempted without authentication")
    ...
    def _decrypt_data(self, encrypted_data: bytes) -> str:
        """데이터 복호화"""
        if not self.auth_manager or not self.auth_manager.is_logged_in():
            security_logger.log_security_violation("decryption_without_auth", "Decryption attempted without authentication")
    ...
    def create_session(self, title: str, topic_category: str = None, model_used: str = None) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            topic_category_encrypted = self._encrypt_data(topic_category) if topic_category else None
            model_used_encrypted = self._encrypt_data(model_used) if model_used else None
            ...
            ''', (title_encrypted, topic_category_encrypted, model_used_encrypted, 
                  self.CURRENT_ENCRYPTION_VERSION))
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                return {
                    ...
                    'topic_category': self._decrypt_data(row['topic_category']) if row['topic_category'] else None,
                    'created_at': row['created_at'],
                    'last_used_at': row['last_used_at'],
                    'message_count': row['actual_message_count'],
                    'model_used': self._decrypt_data(row['model_used']) if row['model_used'] else None,
    ...
    def add_message(self, session_id: int, role: str, content: str, 
                   content_html: str = None, token_count: int = 0, 
                   tool_calls: str = None) -> int:
        """메시지 추가"""
        with sqlite3.connect(self.db_path) as conn:
            # 데이터 암호화
            content_encrypted = self._encrypt_data(content)
            content_html_encrypted = self._encrypt_data(content_html) if content_html else None
            tool_calls_encrypted = self._encrypt_data(tool_calls) if tool_calls else None
            ...
            ''', (session_id, role, content_encrypted, content_html_encrypted,
                  token_count, tool_calls_encrypted, self.CURRENT_ENCRYPTION_VERSION))
    ...
    def get_messages(self, session_id: int, limit: int = 100) -> List[Dict[str, Any]]:
        """메시지 목록 조회"""
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute('''
            ...
            for row in cursor.fetchall():
                try:
                    message = {
                        ...
                        'content_html': self._decrypt_data(row['content_html']) if row['content_html'] else None,
                        'timestamp': row['timestamp'],
                        'token_count': row['token_count'],
                        'tool_calls': self._decrypt_data(row['tool_calls']) if row['tool_calls'] else None,
                        'encryption_version': row['encryption_version']
                    }
    ...
    def get_encryption_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            ''')
            session_stats = {row['encryption_version']: row['count'] for row in cursor.fetchall()}
            ...
            cursor = conn.execute('''
                ...
                GROUP BY encryption_version
            ''')
            message_stats = {row['encryption_version']: row['count'] for row in cursor.fetchall()}
