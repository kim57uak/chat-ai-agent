#!/usr/bin/env python3
"""패키징된 Chat AI Agent 리소스 모니터링 도구"""
...
def find_packaged_chatai_process():
    """패키징된 Chat AI Agent 프로세스 찾기 (프로세스 이름 또는 cmdline 기반)"""
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            # 1. 프로세스 이름으로 찾기 (macOS 앱 번들 이름)
            if proc.info['name'] == 'ChatAIAgent':
                return proc
            
            # 2. cmdline으로 찾기 (더 정확)
            cmdline = proc.info['cmdline']
            if cmdline and any('ChatAIAgent_beta.app/Contents/MacOS/ChatAIAgent' in str(cmd) for cmd in cmdline):
                return proc
...
def monitor_process(interval=2):
    ...
    print("🔍 패키징된 Chat AI Agent 프로세스 검색 중...")
    
    proc = find_packaged_chatai_process()
    if not proc:
        print("❌ 패키징된 Chat AI Agent 프로세스를 찾을 수 없습니다.")
        print("💡 ChatAIAgent_beta.app이 실행 중인지 확인하세요.")
        return
