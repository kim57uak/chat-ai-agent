...
class SessionPanel(QWidget):
    ...
    def search_sessions(self, query: str):
        ...
        if not query.strip():
            ...
            if self.current_session_id:
                QTimer.singleShot(
                    50,
                    lambda: self._select_session_without_touch(self.current_session_id),
                )
            return
    ...
    def select_session(self, session_id: int):
        ...
        # 검색 중이면 검색 결과 유지, 아니면 전체 목록 새로고침 (불필요한 새로고침 제거)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                ...
                QMessageBox.information(
                    self, "성공", f"새 세션 '{data['title']}'이 생성되었습니다."
                )

            except Exception as e:
                logger.error(f"세션 생성 오류: {e}")
                QMessageBox.critical(
                    self, "오류", f"세션 생성 중 오류가 발생했습니다:\n{e}"
                )
        ...
        if ok and new_title.strip():
            try:
                success = session_manager.update_session(
                    self.current_session_id, title=new_title.strip()
                )
                ...
                QMessageBox.critical(
                    self, "오류", f"세션 이름 변경 중 오류가 발생했습니다:\n{e}"
                )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                success = session_manager.delete_session(
                    self.current_session_id, hard_delete=True
                )
                ...
                if success:
                    ...
                    if hasattr(self, "main_window") and self.main_window:
                        if (
                            self.main_window.current_session_id
    ...
    def _apply_material_theme(self):
        ...
        is_dark = theme_manager.is_material_dark_theme()
        header_text_color = (
            colors.get("on_primary", "#000000") if is_dark else "#ffffff"
        )
        ...
        is_dark = theme_manager.is_material_dark_theme()
        stats_text_color = (
            colors.get("text_secondary", "#b3b3b3")
            if is_dark
            else colors.get("text_primary", "#333333")
        )
            ...
            if main_window and hasattr(main_window, "chat_widget"):
                ...
                if hasattr(chat_widget, "input_text"):
                    ...
                    if current_text.strip():
                        # 기존 내용이 있으면 줄바꿈 후 추가
                        chat_widget.input_text.setPlainText(
                            current_text + "\n" + content
                        )
                    else:
                        # 비어있으면 바로 입력
                    ...
                    logger.debug(
                        f"템플릿 내용이 채팅창에 입력되었습니다: {content[:50]}..."
                    )
                else:
    ...
    def _get_improved_display_name(self, model_name, model_config):
        ...
        if description:
            ...
            # 모델명 단순화
            simple_name = (
                model_name.split("/")[-1].replace(":free", "").replace("-instruct", "")
            )
    ...
    def show_tools_detail(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "오류", f"MCP 서버 관리 화면을 열 수 없습니다:\n{e}"
            )
    ...
    def _auto_select_last_session(self):
        ...
        try:
            ...
            if sessions:
                ...
                logger.debug(
                    f"[자동선택] 마지막 세션 발견: {last_session['title']} (메시지 {message_count}개)"
                )
