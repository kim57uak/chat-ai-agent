...
class SystemPromptDialog(QDialog):
    ...
    def on_model_changed(self):
        ...
        if model_type == 'gpt':
            prompt = self.ai_client.system_prompt.get('gpt', '')
        elif model_type == 'gemini':
            prompt = self.ai_client.system_prompt.get('gemini', '')
        else:  # 공통
            # GPT 프롬프트를 기본으로 표시
            prompt = self.ai_client.system_prompt.get('gpt', '')
    ...
    def save_prompt(self):
        ...
        if not prompt_text:
            QMessageBox.warning(self, '경고', '시스템 프롬프트를 입력해주세요.')
            return
        ...
        try:
            self.ai_client.update_system_prompt(prompt_text, model_type)
            # 저장 후 즉시 새로고침
            self.ai_client.system_prompt = self.ai_client._load_system_prompt()
            QMessageBox.information(self, '성공', '시스템 프롬프트가 저장되었습니다.')
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, '오류', f'저장 중 오류가 발생했습니다: {str(e)}')
            print(f'시스템 프롬프트 저장 오류: {str(e)}')
    
    def reset_to_default(self):
        """기본값으로 복원"""
        reply = QMessageBox.question(
            self, '확인', 
            '기본 시스템 프롬프트로 복원하시겠습니까?',
